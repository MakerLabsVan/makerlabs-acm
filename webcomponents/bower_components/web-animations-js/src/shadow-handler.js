...
;(function(scope) {
  ...
  function mergeShadow(left, right) {
    while (
      left.lengths.length < Math.max(left.lengths.length, right.lengths.length)
    )
      left.lengths.push({ px: 0 })
    while (
      right.lengths.length < Math.max(left.lengths.length, right.lengths.length)
    )
    ...
    for (var i = 0; i < left.lengths.length; i++) {
      var mergedDimensions = scope.mergeDimensions(
        left.lengths[i],
        right.lengths[i],
        i == 2
      )
  ...
  function mergeNestedRepeatedShadow(nestedMerge, separator, left, right) {
    ...
    return scope.mergeNestedRepeated(
      nestedMerge,
      separator,
      leftCopy,
      rightCopy
  ...
  scope.addPropertiesHandler(parseShadowList, mergeShadowList, [
    'box-shadow',
    'text-shadow'
  ])
})(webAnimations1)
