...
;(function(shared, scope, testing) {
  ...
  scope.AnimationTimeline.prototype = {
    ...
    _updateAnimationsPromises: function() {
      scope.animationsWithPromises = scope.animationsWithPromises.filter(
        function(animation) {
  ...
  function webAnimationsNextTick(t) {
    ...
    if (timeline._animations.length == 0) ticking = false
    else requestAnimationFrame(webAnimationsNextTick)
  }
