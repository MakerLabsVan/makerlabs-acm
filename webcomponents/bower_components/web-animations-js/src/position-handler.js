...
;(function(scope) {
  ...
  function consumeOffset(string) {
    return (
      scope.consumeToken(/^(left|center|right|top|bottom)\b/i, string) ||
      scope.consumeLengthOrPercent(string)
    )
  }
  ...
  var mergeOffsetList = scope.mergeNestedRepeated.bind(
    null,
    scope.mergeDimensions,
    ' '
  )
  scope.addPropertiesHandler(parseOrigin.bind(null, 3), mergeOffsetList, [
    'transform-origin'
  ])
  scope.addPropertiesHandler(parseOrigin.bind(null, 2), mergeOffsetList, [
    'perspective-origin'
  ])
  ...
  var mergePositionList = scope.mergeNestedRepeated.bind(
    null,
    mergeOffsetList,
    ', '
  )
  scope.addPropertiesHandler(parsePositionList, mergePositionList, [
    'background-position',
    'object-position'
  ])
})(webAnimations1)
