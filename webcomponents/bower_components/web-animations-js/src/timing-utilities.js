...
;(function(shared, testing) {
  ...
  AnimationEffectTiming.prototype = {
    _setMember: function(member, value) {
      ...
      if (this._effect) {
        ...
        this._effect.activeDuration = shared.calculateActiveDuration(
    ...
    set duration(value) {
      if (
        value != 'auto' &&
        (isNaN(value) || value < 0) &&
        isInvalidTimingDeprecated()
      ) {
        throw new TypeError(
          'duration must be non-negative or auto, received: ' + value
        )
      }
    ...
    set iterations(value) {
      if ((isNaN(value) || value < 0) && isInvalidTimingDeprecated()) {
        throw new TypeError(
          'iterations must be non-negative, received: ' + value
        )
      }
  ...
  function parseEasingFunction(normalizedEasing) {
    ...
    if (stepData) {
      return step(
        Number(stepData[1]),
        { start: Start, middle: Middle, end: End }[stepData[2]]
      )
    }
  ...
  function calculateIterationProgress(activeDuration, localTime, timing) {
    ...
    var activeTime = calculateActiveTime(
      activeDuration,
      timing.fill,
      localTime,
      phase,
    ...
    var simpleIterationProgress = calculateSimpleIterationProgress(
      overallProgress,
      timing.iterationStart,
      phase,
      timing.iterations,
    ...
    var currentIteration = calculateCurrentIteration(
      phase,
      timing.iterations,
      simpleIterationProgress,
      overallProgress
    )
    var directedProgress = calculateDirectedProgress(
      timing.direction,
      currentIteration,
      simpleIterationProgress
    )
