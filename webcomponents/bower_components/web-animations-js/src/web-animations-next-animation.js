  ...
  scope.Animation.prototype = {
    ...
    _rebuildUnderlyingAnimation: function() {
      ...
      if (!this.effect || this.effect instanceof window.KeyframeEffect) {
        this._animation = scope.newUnderlyingAnimationForKeyframeEffect(
          this.effect
        )
    ...
    _updateChildren: function() {
      if (!this.effect || this.playState == 'idle') return
    ...
    _setExternalAnimation: function(animation) {
      if (!this.effect || !this._isGroup) return
    ...
    _constructChildAnimations: function() {
      ...
      this.effect.children.forEach(
        function(child) {
          ...
          if (this._paused) childAnimation.pause()
    ...
    _arrangeChildren: function(childAnimation, offset) {
      if (this.startTime === null) {
        childAnimation.currentTime =
          this.currentTime - offset / this.playbackRate
      } else if (
        childAnimation.startTime !==
        this.startTime + offset / this.playbackRate
      ) {
    ...
    set currentTime(v) {
      this._updatePromises()
      this._animation.currentTime = isFinite(v)
        ? v
        : Math.sign(v) * Number.MAX_VALUE
      this._register()
    ...
    set startTime(v) {
      this._updatePromises()
      this._animation.startTime = isFinite(v)
        ? v
        : Math.sign(v) * Number.MAX_VALUE
      this._register()
      this._forEachChild(function(child, offset) {
    ...
    removeEventListener: function(type, handler) {
      this._animation.removeEventListener(
        type,
        (handler && handler._wrapper) || handler
      )
    },
    _removeChildAnimations: function() {
      while (this._childAnimations.length) this._childAnimations.pop().cancel()
    },
    _forEachChild: function(f) {
      ...
      if (this.playState == 'pending') return
      var timing = this.effect._timing
      var t = this.currentTime
      if (t !== null)
        t = shared.calculateIterationProgress(
          shared.calculateActiveDuration(timing),
          t,
          timing
        )
      if (t == null || isNaN(t)) this._removeChildAnimations()
    }
  }
