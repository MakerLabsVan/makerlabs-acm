...
function tap(element) {
  element.dispatchEvent(
    new CustomEvent('tap', { bubbles: true, detail: {}, composed: true })
  )
}

function click(element) {
  element.dispatchEvent(
    new CustomEvent('click', { bubbles: true, detail: {}, composed: true })
  )
}
...
function getFirstVisibleItem(scroller, bufferOffset) {
  ...
  return children.reduce(function(prev, current) {
    return Math.floor(current.getBoundingClientRect().top) -
      Math.floor(scrollerRect.top + bufferOffset) <=
      0
      ? current
      : prev
  })
