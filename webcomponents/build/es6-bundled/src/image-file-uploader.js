define(["../node_modules/@polymer/lit-element/lit-element.js"], function (_litElement) {
  "use strict";

  class ImageFileUploader extends _litElement.LitElement {
    ...
    firstUpdated(changedProperties) {
      ...
      const el = this.shadowRoot.getElementById("drop_zone"); // Draw a border around the drop zone when dragged over

      el.addEventListener("dragover", this.handleDragOver.bind(this), false); // Cancel the border when no longer dragged over

      el.addEventListener("dragleave", this.handleDragLeave.bind(this), false); // Begin the file upload immediately after dropping a file

      el.addEventListener("drop", this.handleDraggedFileUpload.bind(this), false); // Begin the file upload after selecting a file with the chooser dialog

      el.addEventListener("change", this.handleSelectedFileUpload.bind(this), false);
    }
      ...
      if (this.accessToken && files) {
        for (var i = 0, f; f = files[i]; i++) {
          const uploader = new MediaUploader({
            ...
            onComplete: json => {
              this.handleDragLeave(); // Parse the uploaded file response metadata

              const data = JSON.parse(json); // Check if a valid web content link was created/found

              if (data && data.id) {
                var src = `https://drive.google.com/thumbnail?id=${data.id}&sz=w300-c`; // Update displayed image
