...
</iron-iconset-svg>`;document.head.appendChild(template$1.content);Polymer$1({_template:html`
    <style>
      :host {
        display: inline-block;
        overflow: hidden;
...
<dom-module id="paper-material-styles">
  ...
  <slot></slot>`;template$5.setAttribute("strip-whitespace","");Polymer$1({_template:template$5,is:"paper-button",behaviors:[PaperButtonBehavior],properties:{raised:{type:Boolean,reflectToAttribute:!0,value:!1,observer:"_calculateElevation"}},_calculateElevation:function(){if(!this.raised){this._setElevation(0)}else{PaperButtonBehaviorImpl._calculateElevation.apply(this)}}});const template$6=html`
<custom-style>
  <style is="custom-style">
    html {
...
</custom-style>`;template$7.setAttribute("style","display: none;");document.head.appendChild(template$7.content);Polymer$1({_template:html`
    <style include="paper-material-styles">
      :host {
        display: inline-block;
        position: relative;
...
</custom-style>`;template$9.setAttribute("style","display: none;");document.head.appendChild(template$9.content);const $_documentContainer=document.createElement("template");$_documentContainer.setAttribute("style","display: none;");$_documentContainer.innerHTML=`<dom-module id="paper-dialog-shared-styles">
  <template>
    <style>
      :host {
        display: block;
...
`;template$10.setAttribute("style","display: none;");document.head.appendChild(template$10.content);Polymer$1({_template:html`
    <style>
      :host {
        display: block;
        padding: 8px 0;
...
`,is:"paper-input-error",behaviors:[PaperInputAddonBehavior],properties:{invalid:{readOnly:!0,reflectToAttribute:!0,type:Boolean}},update:function(state){this._setInvalid(state.invalid)}});const PaperInputHelper={};_exports.PaperInputHelper=PaperInputHelper;PaperInputHelper.NextLabelID=1;PaperInputHelper.NextAddonID=1;PaperInputHelper.NextInputID=1;const PaperInputBehaviorImpl={properties:{label:{type:String},value:{notify:!0,type:String},disabled:{type:Boolean,value:!1},invalid:{type:Boolean,value:!1,notify:!0},allowedPattern:{type:String},type:{type:String},list:{type:String},pattern:{type:String},required:{type:Boolean,value:!1},errorMessage:{type:String},charCounter:{type:Boolean,value:!1},noLabelFloat:{type:Boolean,value:!1},alwaysFloatLabel:{type:Boolean,value:!1},autoValidate:{type:Boolean,value:!1},validator:{type:String},autocomplete:{type:String,value:"off"},autofocus:{type:Boolean,observer:"_autofocusChanged"},inputmode:{type:String},minlength:{type:Number},maxlength:{type:Number},min:{type:String},max:{type:String},step:{type:String},name:{type:String},placeholder:{type:String,value:""},readonly:{type:Boolean,value:!1},size:{type:Number},autocapitalize:{type:String,value:"none"},autocorrect:{type:String,value:"off"},autosave:{type:String},results:{type:Number},accept:{type:String},multiple:{type:Boolean},_ariaDescribedBy:{type:String,value:""},_ariaLabelledBy:{type:String,value:""},_inputId:{type:String,value:""}},listeners:{"addon-attached":"_onAddonAttached"},keyBindings:{"shift+tab:keydown":"_onShiftTabDown"},hostAttributes:{tabindex:0},get inputElement(){if(!this.$){this.$={}}if(!this.$.input){this._generateInputId();this.$.input=this.$$("#"+this._inputId)}return this.$.input},get _focusableElement(){return this.inputElement},created:function(){this._typesThatHaveText=["date","datetime","datetime-local","month","time","week","file"]},attached:function(){this._updateAriaLabelledBy();if(!PolymerElement&&this.inputElement&&-1!==this._typesThatHaveText.indexOf(this.inputElement.type)){this.alwaysFloatLabel=!0}},_appendStringWithSpace:function(str,more){if(str){str=str+" "+more}else{str=more}return str},_onAddonAttached:function(event){var target=dom(event).rootTarget;if(target.id){this._ariaDescribedBy=this._appendStringWithSpace(this._ariaDescribedBy,target.id)}else{var id="paper-input-add-on-"+PaperInputHelper.NextAddonID++;target.id=id;this._ariaDescribedBy=this._appendStringWithSpace(this._ariaDescribedBy,id)}},validate:function(){return this.inputElement.validate()},_focusBlurHandler:function(event){IronControlState._focusBlurHandler.call(this,event);if(this.focused&&!this._shiftTabPressed&&this._focusableElement){this._focusableElement.focus()}},_onShiftTabDown:function(){var oldTabIndex=this.getAttribute("tabindex");this._shiftTabPressed=!0;this.setAttribute("tabindex","-1");this.async(function(){this.setAttribute("tabindex",oldTabIndex);this._shiftTabPressed=!1},1)},_handleAutoValidate:function(){if(this.autoValidate)this.validate()},updateValueAndPreserveCaret:function(newValue){try{var start=this.inputElement.selectionStart;this.value=newValue;this.inputElement.selectionStart=start;this.inputElement.selectionEnd=start}catch(e){this.value=newValue}},_computeAlwaysFloatLabel:function(alwaysFloatLabel,placeholder){return placeholder||alwaysFloatLabel},_updateAriaLabelledBy:function(){var label=dom(this.root).querySelector("label");if(!label){this._ariaLabelledBy="";return}var labelledBy;if(label.id){labelledBy=label.id}else{labelledBy="paper-input-label-"+PaperInputHelper.NextLabelID++;label.id=labelledBy}this._ariaLabelledBy=labelledBy},_generateInputId:function(){if(!this._inputId||""===this._inputId){this._inputId="input-"+PaperInputHelper.NextInputID++}},_onChange:function(event){if(this.shadowRoot){this.fire(event.type,{sourceEvent:event},{node:this,bubbles:event.bubbles,cancelable:event.cancelable})}},_autofocusChanged:function(){if(this.autofocus&&this._focusableElement){var activeElement=document.activeElement,isActiveElementValid=activeElement instanceof HTMLElement,isSomeElementActive=isActiveElementValid&&activeElement!==document.body&&activeElement!==document.documentElement;if(!isSomeElementActive){this._focusableElement.focus()}}}};_exports.PaperInputBehaviorImpl=PaperInputBehaviorImpl;const PaperInputBehavior=[IronControlState,IronA11yKeysBehavior,PaperInputBehaviorImpl];_exports.PaperInputBehavior=PaperInputBehavior;_exports.$paperInputBehavior={PaperInputHelper:PaperInputHelper,PaperInputBehaviorImpl:PaperInputBehaviorImpl,PaperInputBehavior:PaperInputBehavior};Polymer$1({is:"paper-input",_template:html`
  ...
  `,behaviors:[PaperInputBehavior,IronFormElementBehavior],properties:{value:{type:String}},get _focusableElement(){return this.inputElement._inputElement},listeners:{"iron-input-ready":"_onIronInputReady"},_onIronInputReady:function(){if(!this.$.nativeInput){this.$.nativeInput=this.$$("input")}if(this.inputElement&&-1!==this._typesThatHaveText.indexOf(this.$.nativeInput.type)){this.alwaysFloatLabel=!0}if(!!this.inputElement.bindValue){this.$.container._handleValueAndAutoValidate(this.inputElement)}}});Polymer$1({is:"paper-menu-grow-height-animation",behaviors:[NeonAnimationBehavior],configure:function(config){var node=config.node,rect=node.getBoundingClientRect(),height=rect.height;this._effect=new KeyframeEffect(node,[{height:height/2+"px"},{height:height+"px"}],this.timingFromConfig(config));return this._effect}});Polymer$1({is:"paper-menu-grow-width-animation",behaviors:[NeonAnimationBehavior],configure:function(config){var node=config.node,rect=node.getBoundingClientRect(),width=rect.width;this._effect=new KeyframeEffect(node,[{width:width/2+"px"},{width:width+"px"}],this.timingFromConfig(config));return this._effect}});Polymer$1({is:"paper-menu-shrink-width-animation",behaviors:[NeonAnimationBehavior],configure:function(config){var node=config.node,rect=node.getBoundingClientRect(),width=rect.width;this._effect=new KeyframeEffect(node,[{width:width+"px"},{width:width-width/20+"px"}],this.timingFromConfig(config));return this._effect}});Polymer$1({is:"paper-menu-shrink-height-animation",behaviors:[NeonAnimationBehavior],configure:function(config){var node=config.node,rect=node.getBoundingClientRect(),height=rect.height;this.setPrefixedProperty(node,"transformOrigin","0 0");this._effect=new KeyframeEffect(node,[{height:height+"px",transform:"translateY(0)"},{height:height/2+"px",transform:"translateY(-20px)"}],this.timingFromConfig(config));return this._effect}});var config={ANIMATION_CUBIC_BEZIER:"cubic-bezier(.3,.95,.5,1)",MAX_ANIMATION_TIME_MS:400};const PaperMenuButton=Polymer$1({_template:html`
    <style>
      :host {
        display: inline-block;
        position: relative;
...
`;template$11.setAttribute("strip-whitespace","");Polymer$1({_template:template$11,is:"paper-fab",behaviors:[PaperButtonBehavior],properties:{src:{type:String,value:""},icon:{type:String,value:""},mini:{type:Boolean,value:!1,reflectToAttribute:!0},label:{type:String,observer:"_labelChanged"}},_labelChanged:function(){this.setAttribute("aria-label",this.label)},_computeIsIconFab:function(icon,src){return 0<icon.length||0<src.length}});const template$12=html`
...
`;template$12.setAttribute("style","display: none;");document.body.appendChild(template$12.content);Polymer$1({is:"paper-icon-button",hostAttributes:{role:"button",tabindex:"0"},behaviors:[PaperInkyFocusBehavior],properties:{src:{type:String},icon:{type:String},alt:{type:String,observer:"_altChanged"}},_altChanged:function(newValue,oldValue){var label=this.getAttribute("aria-label");if(!label||oldValue==label){this.setAttribute("aria-label",newValue)}}});const $_documentContainer$3=document.createElement("template");$_documentContainer$3.setAttribute("style","display: none;");$_documentContainer$3.innerHTML=`<dom-module id="paper-item-shared-styles">
  <template>
    <style>
      :host, .paper-item {
        display: block;
...
</dom-module>`;document.head.appendChild($_documentContainer$3.content);const PaperItemBehaviorImpl={hostAttributes:{role:"option",tabindex:"0"}};_exports.PaperItemBehaviorImpl=PaperItemBehaviorImpl;const PaperItemBehavior=[IronButtonState,IronControlState,PaperItemBehaviorImpl];_exports.PaperItemBehavior=PaperItemBehavior;_exports.$paperItemBehavior={PaperItemBehaviorImpl:PaperItemBehaviorImpl,PaperItemBehavior:PaperItemBehavior};Polymer$1({_template:html`
...
`,is:"paper-icon-item",behaviors:[PaperItemBehavior]});Polymer$1({_template:html`
    ...
    <slot></slot>
`,is:"paper-item-body"});Polymer$1({_template:html`
...
`,is:"paper-item",behaviors:[PaperItemBehavior]});Polymer$1({_template:html`
    ...
    <slot></slot>
`,is:"paper-listbox",behaviors:[IronMenuBehavior],hostAttributes:{role:"listbox"}});const template$13=html`
<style>
  :host {
    display: inline-block;
    line-height: 0;
...
</dom-module>`;document.head.appendChild($_documentContainer$4.content);const template$14=html`
...
`;template$14.setAttribute("strip-whitespace","");Polymer$1({_template:template$14,is:"paper-spinner-lite",behaviors:[PaperSpinnerBehavior]});Polymer$1({_template:html`
    <style>
      :host {
        display: block;
        position: absolute;
...
`}static get is(){return"vaadin-button"}static get version(){return"2.1.0"}ready(){super.ready();this.setAttribute("role","button");this.$.button.setAttribute("role","presentation");this._addActiveListeners()}disconnectedCallback(){super.disconnectedCallback();if(this.hasAttribute("active")){this.removeAttribute("active")}}_addActiveListeners(){addListener(this,"down",()=>!this.disabled&&this.setAttribute("active",""));addListener(this,"up",()=>this.removeAttribute("active"));this.addEventListener("keydown",e=>!this.disabled&&0<=[13,32].indexOf(e.keyCode)&&this.setAttribute("active",""));this.addEventListener("keyup",()=>this.removeAttribute("active"));this.addEventListener("blur",()=>this.removeAttribute("active"))}get focusElement(){return this.$.button}}_exports.ButtonElement=ButtonElement;customElements.define(ButtonElement.is,ButtonElement);_exports.$vaadinButton={ButtonElement:ButtonElement};class Material extends HTMLElement{static get version(){return"1.2.0"}}_exports.Material=Material;customElements.define("vaadin-material-styles",Material);_exports.$version={Material:Material};const $_documentContainer$5=document.createElement("template");$_documentContainer$5.innerHTML=`<dom-module id="material-color-light">
  <template>
    <style>
      :host {
        /* Text colors */
...
</custom-style>`;document.head.appendChild($_documentContainer$6.content);const link=document.createElement("link");link.rel="stylesheet";link.type="text/css";link.crossOrigin="anonymous";link.href="https://fonts.googleapis.com/css?family=Roboto+Mono:400,700|Roboto:400,300,300italic,400italic,500,500italic,700,700italic";document.head.appendChild(link);const $_documentContainer$7=document.createElement("template");$_documentContainer$7.innerHTML=`<custom-style>
  <style>
    html {
      /* Font family */
...
</dom-module>`;document.head.appendChild($_documentContainer$7.content);const $_documentContainer$8=document.createElement("template");$_documentContainer$8.innerHTML=`<dom-module id="material-button" theme-for="vaadin-button">
  <template>
    <style>
      :host {
        padding: 8px;
...
</dom-module>`;document.head.appendChild($_documentContainer$8.content);class ComboBoxItemElement extends ThemableMixin(PolymerElement){static get template(){return html`
    <style>
      :host {
        display: block;
      }
...
`}static get is(){return"vaadin-combo-box-item"}static get properties(){return{index:Number,item:Object,label:String,selected:{type:Boolean,value:!1,reflectToAttribute:!0},focused:{type:Boolean,value:!1,reflectToAttribute:!0},_itemTemplateInstance:Object}}static get observers(){return["_updateLabel(label, _itemTemplateInstance)","_updateTemplateInstanceVariable(\"index\", index, _itemTemplateInstance)","_updateTemplateInstanceVariable(\"item\", item, _itemTemplateInstance)","_updateTemplateInstanceVariable(\"selected\", selected, _itemTemplateInstance)","_updateTemplateInstanceVariable(\"focused\", focused, _itemTemplateInstance)"]}connectedCallback(){super.connectedCallback();if(!this._itemTemplateInstance){const overlay=this.getRootNode().host.getRootNode().host,dropdown=overlay.__dataHost,comboBoxOverlay=dropdown.getRootNode().host,comboBox=comboBoxOverlay.getRootNode().host;comboBox._ensureTemplatized();if(comboBox._TemplateClass){this._itemTemplateInstance=new comboBox._TemplateClass({});this.$.content.textContent="";this.$.content.appendChild(this._itemTemplateInstance.root)}}}_updateLabel(label,_itemTemplateInstance){if(_itemTemplateInstance===void 0&&this.$.content){this.$.content.textContent=label}}_updateTemplateInstanceVariable(variable,value,_itemTemplateInstance){if(variable===void 0||value===void 0||_itemTemplateInstance===void 0){return}_itemTemplateInstance[variable]=value}}customElements.define(ComboBoxItemElement.is,ComboBoxItemElement);let overlayContentCounter=0;const createOverlayContent=cssText=>{overlayContentCounter++;const is=`vaadin-overlay-content-${overlayContentCounter}`,styledTemplate=document.createElement("template"),style=document.createElement("style");style.textContent=":host { display: block; }"+cssText;styledTemplate.content.appendChild(style);if(window.ShadyCSS){window.ShadyCSS.prepareTemplate(styledTemplate,is)}const klass=(()=>class extends HTMLElement{static get is(){return is}connectedCallback(){if(window.ShadyCSS){window.ShadyCSS.styleElement(this)}if(!this.shadowRoot){this.attachShadow({mode:"open"});this.shadowRoot.appendChild(document.importNode(styledTemplate.content,!0));this.root=this.shadowRoot}}})();customElements.define(klass.is,klass);return document.createElement(is)};class OverlayElement extends ThemableMixin(PolymerElement){static get template(){return html`
    <style>
      :host {
        z-index: 200;
        position: fixed;
...
</dom-module>`;document.head.appendChild($_documentContainer$10.content);const $_documentContainer$11=document.createElement("template");$_documentContainer$11.innerHTML=`<dom-module id="material-menu-overlay">
  <template>
    <style include="material-overlay">
    </style>
  </template>
</dom-module>`;document.head.appendChild($_documentContainer$11.content);const $_documentContainer$12=document.createElement("template");$_documentContainer$12.innerHTML=`<dom-module id="material-combo-box-overlay" theme-for="vaadin-combo-box-overlay">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$12.content);const $_documentContainer$13=document.createElement("template");$_documentContainer$13.innerHTML=`<custom-style>
  <style>
    @font-face {
...
</custom-style>`;document.head.appendChild($_documentContainer$13.content);const $_documentContainer$14=document.createElement("template");$_documentContainer$14.innerHTML=`<dom-module id="material-item" theme-for="vaadin-item">
  <template>
    <style>
      :host {
        display: flex;
...
`}static get is(){return"vaadin-item"}static get version(){return"2.1.0"}}_exports.ItemElement=ItemElement;customElements.define(ItemElement.is,ItemElement);_exports.$vaadinItem={ItemElement:ItemElement};const $_documentContainer$15=document.createElement("template");$_documentContainer$15.innerHTML=`<dom-module id="material-combo-box-item" theme-for="vaadin-combo-box-item">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$15.content);const $_documentContainer$16=document.createElement("template");$_documentContainer$16.innerHTML=`<dom-module id="material-text-field" theme-for="vaadin-text-field">
  <template>
    <style>
      :host {
        display: inline-flex;
...
</dom-module>`;document.head.appendChild($_documentContainer$16.content);const $_documentContainer$17=document.createElement("template");$_documentContainer$17.innerHTML=`<dom-module id="material-field-button">
  <template>
    <style>
...
</dom-module>`;document.head.appendChild($_documentContainer$17.content);const $_documentContainer$18=document.createElement("template");$_documentContainer$18.innerHTML=`<dom-module id="material-combo-box" theme-for="vaadin-combo-box">
  <template>
...
</dom-module>`;document.head.appendChild($_documentContainer$19.content);class DatePickerOverlayContentElement extends ThemableMixin(GestureEventListeners(PolymerElement)){static get template(){return html`
    <style>
      :host {
        display: flex;
        flex-direction: column;
...
</dom-module>`;document.head.appendChild($_documentContainer$20.content);const $_documentContainer$21=document.createElement("template");$_documentContainer$21.innerHTML=`<dom-module id="material-date-picker-overlay" theme-for="vaadin-date-picker-overlay">
  <template>
    <style include="material-overlay">
      :host([fullscreen]) {
        top: 0 !important;
...
</dom-module>`;document.head.appendChild($_documentContainer$21.content);const $_documentContainer$22=document.createElement("template");$_documentContainer$22.innerHTML=`<dom-module id="material-date-picker-month-calendar" theme-for="vaadin-month-calendar">
  <template>
    <style>
      :host {
        color: var(--material-body-text-color);
...
</dom-module>`;document.head.appendChild($_documentContainer$22.content);const $_documentContainer$23=document.createElement("template");$_documentContainer$23.innerHTML=`<dom-module id="material-date-picker" theme-for="vaadin-date-picker">
  <template>
    <style include="material-field-button">
      :host {
        display: inline-flex;
...
</dom-module>`;document.head.appendChild($_documentContainer$23.content);Polymer$1({is:"google-js-api",behaviors:[IronJsonpLibraryBehavior],properties:{libraryUrl:{type:String,value:"https://apis.google.com/js/api.js?onload=%%callback%%"},notifyEvent:{type:String,value:"js-api-load"}},get api(){return gapi}});var _clientLoaded=!1,_statuses={},_loaders={};Polymer$1({is:"google-client-loader",properties:{name:String,version:String,appId:String,apiRoot:String,successEventName:{type:String,value:"google-api-load"},errorEventName:{type:String,value:"google-api-load-error"}},hostAttributes:{hidden:!0},_waiting:!1,get api(){if(window.gapi&&window.gapi.client&&window.gapi.client[this.name]){return window.gapi.client[this.name]}else{}},get auth(){return gapi.auth},ready:function(){this._loader=document.createElement("google-js-api");if(!this.shadowRoot){this.attachShadow({mode:"open"})}this.shadowRoot.appendChild(this._loader);this.listen(this._loader,"js-api-load","_loadClient")},detached:function(){this.unlisten(this._loader,"js-api-load","_loadClient")},_loadClient:function(){gapi.load("client",this._doneLoadingClient.bind(this))},_handleLoadResponse:function(response){if(response&&response.error){_statuses[this.name]="error";this._fireError(response)}else{_statuses[this.name]="loaded";this._fireSuccess()}},_fireSuccess:function(){this.fire(this.successEventName,{name:this.name,version:this.version})},_fireError:function(response){if(response&&response.error){this.fire(this.errorEventName,{name:this.name,version:this.version,error:response.error})}else{this.fire(this.errorEventName,{name:this.name,version:this.version})}},_doneLoadingClient:function(){_clientLoaded=!0;if(!this._waiting){this._loadApi()}},_createSelfRemovingListener:function(eventName){var handler=function(){_loaders[this.name].removeEventListener(eventName,handler);this._loadApi()}.bind(this);return handler},_loadApi:function(){if(_clientLoaded&&this.name&&this.version){this._waiting=!1;if("loaded"==_statuses[this.name]){this._fireSuccess()}else if("loading"==_statuses[this.name]){this._waiting=!0;_loaders[this.name].addEventListener(this.successEventName,this._createSelfRemovingListener(this.successEventName));_loaders[this.name].addEventListener(this.errorEventName,this._createSelfRemovingListener(this.errorEventName))}else if("error"==_statuses[this.name]){this._fireError(null)}else{var root;if(this.apiRoot){root=this.apiRoot}else if(this.appId){root="https://"+this.appId+".appspot.com/_ah/api"}_statuses[this.name]="loading";_loaders[this.name]=this;gapi.client.load(this.name,this.version,this._handleLoadResponse.bind(this),root)}}}});Polymer$1({is:"google-legacy-loader",behaviors:[IronJsonpLibraryBehavior],properties:{libraryUrl:{type:String,value:"https://www.google.com/jsapi?callback=%%callback%%"},notifyEvent:{type:String,value:"api-load"}},get api(){return google}});Polymer$1({is:"google-maps-api",behaviors:[IronJsonpLibraryBehavior],properties:{mapsUrl:{type:String,value:"https://maps.googleapis.com/maps/api/js?callback=%%callback%%"},apiKey:{type:String,value:""},clientId:{type:String,value:""},version:{type:String,value:"3.exp"},language:{type:String,value:""},signedIn:{type:Boolean,value:!1},notifyEvent:{type:String,value:"api-load"},libraryUrl:{type:String,computed:"_computeUrl(mapsUrl, version, apiKey, clientId, language, signedIn)"}},_computeUrl:function(mapsUrl,version,apiKey,clientId,language,signedIn){var url=mapsUrl+"&v="+version;url+="&libraries=drawing,geometry,places,visualization";if(apiKey&&!clientId){url+="&key="+apiKey}if(clientId){url+="&client="+clientId}if(!apiKey&&!clientId){console.warn("No Google Maps API Key or Client ID specified. "+"See https://developers.google.com/maps/documentation/javascript/get-api-key "+"for instructions to get started with a key or client id.")}if(language){url+="&language="+language}if(signedIn){url+="&signed_in="+signedIn}return url},get api(){return google.maps}});Polymer$1({is:"google-plusone-api",behaviors:[IronJsonpLibraryBehavior],properties:{libraryUrl:{type:String,value:"https://apis.google.com/js/plusone.js?onload=%%callback%%"},notifyEvent:{type:String,value:"api-load"}},get api(){return gapi}});Polymer$1({is:"google-realtime-api",behaviors:[IronJsonpLibraryBehavior],properties:{libraryUrl:{type:String,value:"https://apis.google.com/js/drive-realtime.js?onload=%%callback%%"},notifyEvent:{type:String,value:"api-load"}},get api(){return gapi.drive.realtime}});Polymer$1({is:"google-youtube-api",behaviors:[IronJsonpLibraryBehavior],properties:{libraryUrl:{type:String,value:"https://www.youtube.com/iframe_api"},notifyEvent:{type:String,value:"api-load"},callbackName:{type:String,value:"onYouTubeIframeAPIReady"}},get api(){return YT}});!function(root,factory){if("object"===typeof module&&module.exports){module.exports=factory(root);module.exports["default"]=module.exports}else root.timeago=factory(root)}("undefined"!==typeof window?window:void 0,function(){var indexMapEn="second_minute_hour_day_week_month_year".split("_"),indexMapZh="\u79D2_\u5206\u949F_\u5C0F\u65F6_\u5929_\u5468_\u6708_\u5E74".split("_"),locales={en:function(number,index){if(0===index)return["just now","right now"];var unit=indexMapEn[parseInt(index/2)];if(1<number)unit+="s";return[number+" "+unit+" ago","in "+number+" "+unit]},zh_CN:function(number,index){if(0===index)return["\u521A\u521A","\u7247\u523B\u540E"];var unit=indexMapZh[parseInt(index/2)];return[number+unit+"\u524D",number+unit+"\u540E"]}},SEC_ARRAY=[60,60,24,7,365/7/12,12],SEC_ARRAY_LEN=6,ATTR_DATA_TID="data-tid",timers={};function toDate(input){if(input instanceof Date)return input;if(!isNaN(input))return new Date(toInt(input));if(/^\d+$/.test(input))return new Date(toInt(input));input=(input||"").trim().replace(/\.\d+/,"").replace(/-/,"/").replace(/-/,"/").replace(/(\d)T(\d)/,"$1 $2").replace(/Z/," UTC").replace(/([\+\-]\d\d)\:?(\d\d)/," $1$2");return new Date(input)}function toInt(f){return parseInt(f)}function formatDiff(diff,locale,defaultLocale){locale=locales[locale]?locale:locales[defaultLocale]?defaultLocale:"en";var i=0,agoin=0>diff?1:0,total_sec=diff=_Mathabs(diff);for(;diff>=SEC_ARRAY[i]&&i<SEC_ARRAY_LEN;i++){diff/=SEC_ARRAY[i]}diff=toInt(diff);i*=2;if(diff>(0===i?9:1))i+=1;return locales[locale](diff,i,total_sec)[agoin].replace("%s",diff)}function diffSec(date,nowDate){nowDate=nowDate?toDate(nowDate):new Date;return(nowDate-toDate(date))/1e3}function nextInterval(diff){var rst=1,i=0,d=_Mathabs(diff);for(;diff>=SEC_ARRAY[i]&&i<SEC_ARRAY_LEN;i++){diff/=SEC_ARRAY[i];rst*=SEC_ARRAY[i]}d=d%rst;d=d?rst-d:rst;return _Mathceil(d)}function getDateAttr(node){return getAttr(node,"data-timeago")||getAttr(node,"datetime")}function getAttr(node,name){if(node.getAttribute)return node.getAttribute(name);if(node.attr)return node.attr(name)}function setTidAttr(node,val){if(node.setAttribute)return node.setAttribute(ATTR_DATA_TID,val);if(node.attr)return node.attr(ATTR_DATA_TID,val)}function Timeago(nowDate,defaultLocale){this.nowDate=nowDate;this.defaultLocale=defaultLocale||"en"}Timeago.prototype.doRender=function(node,date,locale){var diff=diffSec(date,this.nowDate),self=this,tid;node.innerHTML=formatDiff(diff,locale,this.defaultLocale);timers[tid=setTimeout(function(){self.doRender(node,date,locale);delete timers[tid]},_Mathmin(1e3*nextInterval(diff),2147483647))]=0;setTidAttr(node,tid)};Timeago.prototype.format=function(date,locale){return formatDiff(diffSec(date,this.nowDate),locale,this.defaultLocale)};Timeago.prototype.render=function(nodes,locale){if(nodes.length===void 0)nodes=[nodes];for(var i=0,len=nodes.length;i<len;i++){this.doRender(nodes[i],getDateAttr(nodes[i]),locale)}};Timeago.prototype.setLocale=function(locale){this.defaultLocale=locale};function timeagoFactory(nowDate,defaultLocale){return new Timeago(nowDate,defaultLocale)}timeagoFactory.register=function(locale,localeFunc){locales[locale]=localeFunc};timeagoFactory.cancel=function(node){if(node){tid=getAttr(node,ATTR_DATA_TID);if(tid){clearTimeout(tid);delete timers[tid]}}else{for(var tid in timers)clearTimeout(tid);timers={}}};return timeagoFactory});class UserSearchBar extends LitElement{static get properties(){return{items:{type:Array}}}constructor(){super();this.items=[]}render(){return html$1`
      <style>
        :host {
      ...
      `}handleSearchChanged(){const searchbox=this.shadowRoot.getElementById("searchbox");if(searchbox&&searchbox.selectedItem){this.dispatchEvent(new CustomEvent("search",{detail:babelHelpers.objectSpread({},searchbox.selectedItem)}))}}resetSelectedItem(){const searchbox=this.shadowRoot.getElementById("searchbox");if(searchbox&&searchbox.selectedItem){searchbox.selectedItem=null}}mergeItem(item){const searchbox=this.shadowRoot.getElementById("searchbox");if(item&&item.type&&item.MakerLabs_ID){for(var found=!1,i=0;i<this.items.length;++i){if(this.items[i].type&&this.items[i].type==item.type&&this.items[i].MakerLabs_ID&&this.items[i].MakerLabs_ID===item.MakerLabs_ID){this.items[i]=item;if(searchbox&&searchbox.selectedItem&&searchbox.selectedItem.MakerLabs_ID===item.MakerLabs_ID){searchbox.selectedItem=this.items[i]}found=!0;break}}if(!found){this.items.push(item)}this.requestUpdate()}}}customElements.define("user-search-bar",UserSearchBar);class ImageFileUploader extends LitElement{static get properties(){return{src:{type:String},name:{type:String},emptyImageData:{type:String},parentFolderId:{type:String}}}constructor(){super();this.emptyImageData="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=";this.src=this.emptyImageData;this.parentFolderId="root"}render(){return html$1`
      <style>
        #drop_zone {
          height: 250px;
    ...
    `}updated(){const el=this.shadowRoot.getElementById("drop_zone");el.addEventListener("dragover",this.handleDragOver.bind(this),!1);el.addEventListener("dragleave",this.handleDragLeave.bind(this),!1);el.addEventListener("drop",this.handleFileUpload.bind(this),!1);el.addEventListener("change",this.handleFileUpload.bind(this),!1)}get accessToken(){var accessToken=null;const authInstance=gapi&&gapi.auth2&&gapi.auth2.getAuthInstance();if(authInstance){const currentUser=authInstance&&authInstance.currentUser&&authInstance.currentUser.get();if(currentUser){const authResponse=currentUser.getAuthResponse(!0);if(authResponse){if("access_token"in authResponse){accessToken=authResponse.access_token}}}}return accessToken}uploadFiles(files){if(this.accessToken){for(var i=0,f;f=files[i];i++){const uploader=new MediaUploader({file:f,metadata:{title:f.name,mimeType:f.type,parents:[{id:this.parentFolderId}]},token:this.accessToken,onComplete:json=>{this.handleDragLeave();const data=JSON.parse(json);if(data&&data.id){var src=`https://drive.google.com/thumbnail?id=${data.id}&sz=w300-c`;this.src=src;this.clearUploadedFiles()}}});uploader.upload()}}else{console.log("Missing accessToken")}}clearUploadedFiles(){const el=this.shadowRoot.getElementById("file-input");if(el&&el.value){el.value=""}}handleFileUpload(evt){evt.stopPropagation();evt.preventDefault();this.uploadFiles(evt.target.files)}handleDragOver(evt){const el=this.shadowRoot.getElementById("drop_zone");if(el){el.style.borderWidth="2px"}const hint=this.shadowRoot.getElementById("drop_hint");if(hint){hint.style.display="block"}if(evt){evt.stopPropagation();evt.preventDefault();evt.dataTransfer.dropEffect="copy"}}handleDragLeave(evt){const el=this.shadowRoot.getElementById("drop_zone");if(el){el.style.borderWidth="0px"}const hint=this.shadowRoot.getElementById("drop_hint");if(hint){hint.style.display="none"}if(evt){evt.stopPropagation();evt.preventDefault()}}}customElements.define("image-file-uploader",ImageFileUploader);var RetryHandler=function(){this.interval=1e3;this.maxInterval=1e3*60};RetryHandler.prototype.retry=function(fn){setTimeout(fn,this.interval);this.interval=this.nextInterval_()};RetryHandler.prototype.reset=function(){this.interval=1e3};RetryHandler.prototype.nextInterval_=function(){var interval=2*this.interval+this.getRandomInt_(0,1e3);return _Mathmin(interval,this.maxInterval)};RetryHandler.prototype.getRandomInt_=function(min,max){return _Mathfloor(Math.random()*(max-min+1)+min)};var MediaUploader=function(options){var noop=function(){};this.file=options.file;this.contentType=options.contentType||this.file.type||"application/octet-stream";this.metadata=options.metadata||{title:this.file.name,mimeType:this.contentType};this.token=options.token;this.onComplete=options.onComplete||noop;this.onProgress=options.onProgress||noop;this.onError=options.onError||noop;this.offset=options.offset||0;this.chunkSize=options.chunkSize||0;this.retryHandler=new RetryHandler;this.url=options.url;if(!this.url){var params=options.params||{};params.uploadType="resumable";this.url=this.buildUrl_(options.fileId,params,options.baseUrl)}this.httpMethod=options.fileId?"PUT":"POST"};MediaUploader.prototype.upload=function(){var self=this,xhr=new XMLHttpRequest;xhr.open(this.httpMethod,this.url,!0);xhr.setRequestHeader("Authorization","Bearer "+this.token);xhr.setRequestHeader("Content-Type","application/json");xhr.setRequestHeader("X-Upload-Content-Length",this.file.size);xhr.setRequestHeader("X-Upload-Content-Type",this.contentType);xhr.onload=function(e){if(400>e.target.status){var location=e.target.getResponseHeader("Location");this.url=location;this.sendFile_()}else{this.onUploadError_(e)}}.bind(this);xhr.onerror=this.onUploadError_.bind(this);xhr.send(JSON.stringify(this.metadata))};MediaUploader.prototype.sendFile_=function(){var content=this.file,end=this.file.size;if(this.offset||this.chunkSize){if(this.chunkSize){end=_Mathmin(this.offset+this.chunkSize,this.file.size)}content=content.slice(this.offset,end)}var xhr=new XMLHttpRequest;xhr.open("PUT",this.url,!0);xhr.setRequestHeader("Content-Type",this.contentType);xhr.setRequestHeader("Content-Range","bytes "+this.offset+"-"+(end-1)+"/"+this.file.size);xhr.setRequestHeader("X-Upload-Content-Type",this.file.type);if(xhr.upload){xhr.upload.addEventListener("progress",this.onProgress)}xhr.onload=this.onContentUploadSuccess_.bind(this);xhr.onerror=this.onContentUploadError_.bind(this);xhr.send(content)};MediaUploader.prototype.resume_=function(){var xhr=new XMLHttpRequest;xhr.open("PUT",this.url,!0);xhr.setRequestHeader("Content-Range","bytes */"+this.file.size);xhr.setRequestHeader("X-Upload-Content-Type",this.file.type);if(xhr.upload){xhr.upload.addEventListener("progress",this.onProgress)}xhr.onload=this.onContentUploadSuccess_.bind(this);xhr.onerror=this.onContentUploadError_.bind(this);xhr.send()};MediaUploader.prototype.extractRange_=function(xhr){var range=xhr.getResponseHeader("Range");if(range){this.offset=parseInt(range.match(/\d+/g).pop(),10)+1}};MediaUploader.prototype.onContentUploadSuccess_=function(e){if(200==e.target.status||201==e.target.status){this.onComplete(e.target.response)}else if(308==e.target.status){this.extractRange_(e.target);this.retryHandler.reset();this.sendFile_()}else{this.onContentUploadError_(e)}};MediaUploader.prototype.onContentUploadError_=function(e){if(e.target.status&&500>e.target.status){this.onError(e.target.response)}else{this.retryHandler.retry(this.resume_.bind(this))}};MediaUploader.prototype.onUploadError_=function(e){this.onError(e.target.response)};MediaUploader.prototype.buildQuery_=function(params){params=params||{};return Object.keys(params).map(function(key){return encodeURIComponent(key)+"="+encodeURIComponent(params[key])}).join("&")};MediaUploader.prototype.buildUrl_=function(id,params,baseUrl){var url=baseUrl||"https://www.googleapis.com/upload/drive/v2/files/";if(id){url+=id}var query$$1=this.buildQuery_(params);if(query$$1){url+="?"+query$$1}return url};function beep(){var snd=new Audio("data:audio/wav;base64,");snd.play()}_exports.$beep={default:beep};class ViewUserForm extends LitElement{static get properties(){return{sheetId:{type:String},nextMakerLabsIdUrl:{type:String},machineId:{type:String},accessToken:{type:String},fields:{type:Array},userName:{type:String,observer:"handleUserNameChanged"},query:{type:Object},defaultPhotoUrl:{type:String},profilePhotosFolderId:{type:String},isSaving:{type:Boolean}}}constructor(){super();this.fields=[];this.query={};this.isSaving=!1;if(window&&window.location){const url=new URL(window.location),queryParams=new URLSearchParams(url.search.slice(1));for(var[k,v]of queryParams){this.query[k]=v}}}updated(changedProperties){super.updated(changedProperties);if(changedProperties.has("fields")){for(var datepickers=this.shadowRoot.querySelectorAll("vaadin-date-picker"),d=0;d<datepickers.length;d++){const datepicker=datepickers[d];datepicker.checkValidity=value=>{const parts=value&&value.split("-");return!value||0===value.length||3==parts.length&&4==parts[0].length&&2==parts[1].length&&2==parts[2].length};datepicker.i18n.formatDate=d=>{const yearStr=(d.year+"").replace(/\d+/,y=>"0000".substr(y.length)+y),month=d.month+1,monthStr=(month+"").replace(/\d+/,m=>"00".substr(m.length)+m),dayStr=(d.day+"").replace(/\d+/,d=>"00".substr(d.length)+d);return[yearStr,monthStr,dayStr].join("-")};datepicker.i18n.parseDate=text=>{const parts=text.split("-"),today=new Date;let date,month=today.getMonth(),year=today.getFullYear();if(3===parts.length){year=parseInt(parts[0]);if(3>parts[0].length&&0<=year){year+=50>year?2e3:1900}month=parseInt(parts[1])-1;date=parseInt(parts[0])}else if(2===parts.length){month=parseInt(parts[1])-1;date=parseInt(parts[0])}else if(1===parts.length){date=parseInt(parts[1])}if(void 0!==date){return{day:date,month,year}}};datepicker.addEventListener("value-changed",e=>{e.target.blur()})}}}render(){return html$1`
      ...
      <form onsubmit="return false;" id="viewUserForm">
        ...
        <div class="mdc-layout-grid">
          <div class="mdc-layout-grid__inner">
            ${this.fields.map(section=>html$1`
                <paper-card
    ...
    `}renderField(field){if(this.fieldIsCheckboxType(field)){return html$1`
        <div>
          <paper-checkbox
            name="${field.name}"
            id="${field.name}"
      ...
      `}if(this.fieldIsRadioGroupType(field)){return html$1`
        <div>
          ...
          <paper-radio-group
            ...
            selected="0"
          >
            ${field.choices.map(choice=>html$1`
                <paper-radio-button
                  name="${choice}"
      ...
      `}if(this.fieldIsDropdownMenuType(field)){return html$1`
        <paper-dropdown-menu
          label="${field.title}"
          name="${field.name}"
          id="${field.name}"
      ...
      `}if(this.fieldIsDatePickerType(field)){return html$1`
        <vaadin-date-picker class="full-width"
          label="${field.title}"
          name="${field.name}"
          id="${field.name}"
        ></vaadin-date-picker>
      `}if(this.fieldIsImageType(field)){return html$1`
        <image-file-uploader
          id="${field.name}"
          name="${field.name}"
          parentFolderId="${this.profilePhotosFolderId}"
        ></image-file-uploader>
      `}if(this.fieldIsTextInputType(field)){return html$1`
        <paper-input
          always-float-label
          label="${field.title}"
          id="${field.name}"
          name="${field.name}"
        ></paper-input>
      `}if(this.fieldIsAlertsType(field)){return html$1`
        <paper-input
          always-float-label
          ...
          name="${field.name}"
        ></paper-input>
      `}if(this.fieldIsTagIdType(field)){return html$1`
        <vaadin-combo-box
          label="${field.title}"
          id="${field.name}"
          name="${field.name}"
      ...
      `}}get usersSheetName(){return"Users"}get usersSheetNumHeaders(){return 2}get activitySheetName(){return"Activity"}get activitySheetNumHeaders(){return 2}get hiddenSectionNames(){return["Initial","Hidden","Private"]}get usersRowColumn(){return this.usersColumnFromFieldTitle("Hidden","Row")}get usersNameColumn(){return this.usersColumnFromFieldTitle("Maker Info","Name")}get usersEmailColumn(){return this.usersColumnFromFieldTitle("Maker Info","Email")}get usersMakerLabsIdColumn(){return this.usersColumnFromFieldTitle("Membership Info","MakerLabs ID")}get usersAlertsColumn(){return this.usersColumnFromFieldTitle("Membership Info","Alerts")}get usersTagIdColumn(){return this.usersColumnFromFieldTitle("Access & Studio","Tag ID")}get activityTimestampColumn(){return"A"}get activityMachineIdColumn(){return"B"}get activityTypeColumn(){return"C"}get activityTagIdColumn(){return"E"}get activityMakerLabsIdColumn(){return"F"}sectionIsHidden(section){return section&&-1!=this.hiddenSectionNames.indexOf(section.title)}fieldIsCheckboxType(field){return"CHECKBOX"===field.type||"VALUE_IN_LIST"===field.type&&field.choices&&2==field.choices.length&&this.isNoLike(field.choices[0])&&this.isYesLike(field.choices[1])}fieldIsRadioGroupType(field){return"VALUE_IN_LIST"===field.type&&field.choices&&3>=field.choices.length&&!this.fieldIsCheckboxType(field)}fieldIsDropdownMenuType(field){return"VALUE_IN_LIST"===field.type&&field.choices&&3<field.choices.length}fieldIsDatePickerType(field){return"DATE_AFTER"==field.type||"DATE_BEFORE"==field.type||"DATE_BETWEEN"==field.type||"DATE_EQUAL_TO"==field.type||"DATE_IS_VALID_DATE"==field.type||"DATE_NOT_BETWEEN"==field.type||"DATE_ON_OR_AFTER"==field.type||"DATE_ON_OR_BEFORE"==field.type}fieldIsTextInputType(field){return!this.fieldIsTagIdType(field)&&!this.fieldIsAlertsType(field)&&(!field.type||"NUMBER_BETWEEN"==field.type||"NUMBER_EQUAL_TO"==field.type||"NUMBER_GREATER_THAN"==field.type||"NUMBER_GREATER_THAN_OR_EQUAL_TO"==field.type||"NUMBER_LESS_THAN"==field.type||"NUMBER_LESS_THAN_OR_EQUAL_TO"==field.type||"NUMBER_NOT_BETWEEN"==field.type||"NUMBER_NOT_EQUAL_TO"==field.type||"TEXT_CONTAINS"==field.type||"TEXT_DOES_NOT_CONTAIN"==field.type||"TEXT_EQUAL_TO"==field.type||"TEXT_IS_VALID_EMAIL"==field.type)}fieldIsAlertsType(field){return field.name===this.usersAlertsColumn}fieldIsTagIdType(field){return field.name===this.usersTagIdColumn}fieldIsImageType(field){return"TEXT_IS_VALID_URL"===field.type}usersColumnFromFieldTitle(prefixOrTitle,title){const prefix=title?prefixOrTitle:"";if(!title){title=prefixOrTitle}if(this.fields&&this.fields.length){for(var s=0,section;s<this.fields.length;++s){section=this.fields[s];for(var f=0,field;f<section.fields.length;++f){field=section.fields[f];if(prefix.length){var prefixStart=field.title.indexOf(prefix);if(0===prefixStart){var titleStart=field.title.indexOf(title,prefixStart+prefix.length+1);if(0===titleStart&&titleStart+title.length===field.length-1){return field.name}}}if(field.title==title){return field.name}}}}return null}handleUserNameChanged(newValue){var _this2=this;return babelHelpers.asyncToGenerator(function*(){if(newValue){if(_this2.usersNameColumn&&_this2.userName){const datatable=yield _this2.queryUsers(`where ${_this2.usersNameColumn} = '${_this2.userName}'`);var values=_this2.getFirstRowValuesFromDatatable(datatable);if(values&&values.length){_this2.showUserRow(values)}}}})()}firstUpdated(){var _this3=this;return babelHelpers.asyncToGenerator(function*(){_this3.updateRecentTagIds();window.setInterval(_this3.updateRecentTagIds.bind(_this3),1e4)})()}updateFormFromMostRecentScan(){var _this4=this;return babelHelpers.asyncToGenerator(function*(){if(_this4.fields&&_this4.usersMakerLabsIdColumn){const datatable=yield _this4.queryActivity("select "+_this4.activityMakerLabsIdColumn+" where "+_this4.activityTypeColumn+" = 'Signed_In' and "+_this4.activityMachineIdColumn+" = '"+_this4.machineId+"' order by "+_this4.activityTimestampColumn+" desc limit 1");var activityValues=_this4.getValuesFromDatatable(datatable);if(activityValues&&activityValues.length){for(var makerLabsIdPrev="",rowIdx=0,makerLabsId;rowIdx<activityValues.length;rowIdx++){makerLabsId=activityValues[rowIdx][0]||"";if(makerLabsId!=makerLabsIdPrev){var didUpdateForm=_this4.updateFormFromMakerLabsId(makerLabsId);if(didUpdateForm){makerLabsIdPrev=makerLabsId}}}}}})()}updateFormFromMakerLabsId(makerLabsId){var _this5=this;return babelHelpers.asyncToGenerator(function*(){const datatable=yield _this5.queryUsers("where "+_this5.usersMakerLabsIdColumn+" = '"+makerLabsId+"'");var userValues=_this5.getFirstRowValuesFromDatatable(datatable);if(userValues&&userValues.length){_this5.showUserRow(userValues);return!0}return!1})()}updateRecentTagIds(){var _this6=this;return babelHelpers.asyncToGenerator(function*(){const el=_this6.shadowRoot.getElementById(_this6.usersTagIdColumn);if(el){const sinceTimestamp=Date.now()-1e3*(60*2),datatable=yield _this6.queryActivity("select "+_this6.activityTagIdColumn+",max("+_this6.activityMakerLabsIdColumn+"),max("+_this6.activityTimestampColumn+") where "+_this6.activityTimestampColumn+" > "+sinceTimestamp+" group by "+_this6.activityTagIdColumn+" limit 5");var items=[],values=_this6.getValuesFromDatatable(datatable);if(values){for(var rowIdx=0;rowIdx<values.length;rowIdx++){var tagId=values[rowIdx][0],makerLabsId=values[rowIdx][1],timestampMillis=values[rowIdx][2],timestamp=new Date(timestampMillis);if(!makerLabsId||0==makerLabsId.length){items.push({tagId,timestamp:timeago().format(timestamp)})}}el.items=items}}})()}getValuesFromDatatable(datatable,limit){var values=[];if(datatable&&datatable.table&&datatable.table.rows&&0<datatable.table.rows.length&&datatable.table.cols&&0<datatable.table.cols.length){limit=limit<datatable.table.rows.length?limit:datatable.table.rows.length;for(var rowIdx=0,rowValues;rowIdx<limit;rowIdx++){rowValues=[];for(var colIdx=0;colIdx<datatable.table.cols.length;colIdx++){var c=datatable.table.rows[rowIdx].c[colIdx],v=c&&(c.f?c.f:c.v);rowValues.push(v)}values.push(rowValues)}}return values}getFirstRowValuesFromDatatable(datatable){const limitFirstRowValues=this.getValuesFromDatatable(datatable,1);return 0<limitFirstRowValues.length?limitFirstRowValues[0]:null}isYesLike(s){return-1!=["\u2611","Y","y","T","t","Yes","yes","True","true"].indexOf(s)}isNoLike(s){return-1!=["\u2610","N","n","F","f","No","no","False","false"].indexOf(s)}arraysEqual(a,b){if(a===b)return!0;if(null==a||null==b)return!1;if(a.length!=b.length)return!1;for(var i=0;i<a.length;++i){if(a[i]!==b[i])return!1}return!0}querySheet(sheetName,query$$1,numHeaders=1){if(!this.accessToken){console.log("Early exit from querySheet, no access token set");return null}var queryString=encodeURIComponent(query$$1);const responseHandlerName=`jsonp_${Date.now()}_${_Mathceil(1e5*Math.random())}`,query_url="https://docs.google.com/spreadsheets/d/"+this.sheetId+"/gviz/tq"+"?access_token="+this.accessToken+"&sheet="+sheetName+"&headers="+numHeaders+"&tq="+queryString+"&tqx=responseHandler:"+responseHandlerName;return fetchJsonp(query_url,{jsonpCallback:"responseHandler",jsonpCallbackFunction:responseHandlerName}).then(function(response){return response.json()}).catch(function(err){console.log("Parsing Google Visualization API response failed",err)})}queryUsers(query$$1){return this.querySheet(this.usersSheetName,query$$1,this.usersSheetNumHeaders)}queryActivity(query$$1){return this.querySheet(this.activitySheetName,query$$1,this.activitySheetNumHeaders)}resetValues(){this.showUserRow([])}populateNewUser(){this.showUserRow([]);this.dispatchEvent(new CustomEvent("reset-form"));this.populateNextMakerLabsId()}showUserObj(user){if(user.alerts){beep()}const nameField=this.fieldForColumnId(this.usersNameColumn);if(nameField&&user.name){this.updateField(nameField,user.name)}const emailField=this.fieldForColumnId(this.emailsNameColumn);if(emailField&&user.email){this.updateField(emailField,user.email)}const makerLabsIdField=this.fieldForColumnId(this.usersMakerLabsIdColumn);if(makerLabsIdField&&user.makerlabsId){this.updateField(makerLabsIdField,user.makerlabsId)}const alertsField=this.fieldForColumnId(this.usersAlertsColumn);if(alertsField&&user.alerts){this.updateField(alertsField,user.alerts)}const tagIdField=this.fieldForColumnId(this.usersTagIdColumn);if(tagIdField&&user.tagId){this.updateField(tagIdField,user.tagId)}}showUserRow(data){for(var i=0,s=0,section;s<this.fields.length;++s){section=this.fields[s];for(var f=0;f<section.fields.length;++f){var val=i<data.length?data[i++]:null,field=section.fields[f];this.updateField(field,val)}}}updateField(field,val){if(field.name){var el=this.shadowRoot.getElementById(field.name);if(!el){console.log("Invalid el for field.name = "+field.name)}if(!val){val=""}if(this.fieldIsCheckboxType(field)){el.checked=this.isYesLike(val[0])}else if(this.fieldIsRadioGroupType(field)){el.select(val)}else if(this.fieldIsDropdownMenuType(field)){var listbox=el.querySelector("paper-listbox"),selectedIdx=field.choices.indexOf(val);listbox.selected=0<=selectedIdx?selectedIdx:0}else if(this.fieldIsDatePickerType(field)){const textField=el.shadowRoot.querySelector("vaadin-text-field");if(textField){textField.value="";textField.blur()}el.value=val}else if(this.fieldIsTextInputType(field)||this.fieldIsAlertsType(field)||this.fieldIsTagIdType(field)){el.value=val}else if(this.fieldIsImageType(field)){var photoUrl=val||this.defaultPhotoUrl;el.src=photoUrl}else{console.log(`Unknown field name ${field.name}, type ${field.type}`)}}else{console.log(`Missing field name`)}}fieldForColumnId(columnId){for(var s=0,section;s<this.fields.length;++s){section=this.fields[s];for(var f=0,field;f<section.fields.length;++f){field=section.fields[f];if(field.name==columnId){return field}}}return null}populateNextMakerLabsId(){var _this7=this;return babelHelpers.asyncToGenerator(function*(){if(!_this7.accessToken||"null"===_this7.accessToken||"undefined"===_this7.accessToken){console.log("Early exit from populateNextMakerLabsId, no access token set");return null}const response=yield fetch(_this7.nextMakerLabsIdUrl,{mode:"cors",credentials:"omit",headers:{Authorization:`Bearer ${_this7.accessToken}`}});if(200==response.status){const nextMakerLabsId=yield response.json(),makerLabsIdField=_this7.fieldForColumnId(_this7.usersMakerLabsIdColumn);if(makerLabsIdField){_this7.updateField(makerLabsIdField,nextMakerLabsId)}}else{console.log(`Fields JSON request fetch failed with response code: ${response.status}`)}})()}handleSubmit(){var _this8=this;return babelHelpers.asyncToGenerator(function*(){for(var sheets=_this8.shadowRoot.getElementById("sheets"),validTextFieldCount=0,validMakerLabsId=!1,user={},formValues=[],s=0,section;s<_this8.fields.length;++s){section=_this8.fields[s];for(var f=0;f<section.fields.length;++f){var formValue,field=section.fields[f],el=_this8.shadowRoot.getElementById(field.name);if(el){if(_this8.fieldIsCheckboxType(field)){formValue=el.checked?field.choices[0]:field.choices[1]}else if(_this8.fieldIsRadioGroupType(field)){formValue=el.selected}else if(_this8.fieldIsDropdownMenuType(field)){formValue=el.value}else if(_this8.fieldIsDatePickerType(field)){let value=el.value;if(!value){const textField=el.shadowRoot.querySelector("vaadin-text-field");if(textField&&textField.value){value=textField.value}}formValue=value}else if(_this8.fieldIsTextInputType(field)){formValue=el.value;if(formValue){if(field.name==_this8.usersMakerLabsIdColumn){validMakerLabsId=!0}else{validTextFieldCount++}}}else if(_this8.fieldIsImageType(field)){if(el.src!=el.emptyImageData){formValue=el.src}}else if(_this8.fieldIsAlertsType(field)){formValue=el.value}else if(_this8.fieldIsTagIdType(field)){formValue=el.value}else{console.log(`Unknown field ${field.name}`)}}user[field.title]=formValue;formValues.push(formValue)}}if(sheets&&sheets.api&&formValues.length&&validMakerLabsId){var rowName=formValues[0];_this8.isSaving=!0;if(rowName){formValues[0]=null;console.log(`Update '${_this8.usersSheetName}' row ${rowName}`);const response=yield sheets.api.spreadsheets.values.update({spreadsheetId:_this8.sheetId,range:`${_this8.usersSheetName}!A${rowName}`,majorDimension:"ROWS",valueInputOption:"USER_ENTERED",values:[formValues]});if(200==response.status){console.log(`Successfully updated '${_this8.usersSheetName}' row ${rowName}`);user.Row=parseInt(rowName);_this8.dispatchEvent(new CustomEvent("updated-user",{detail:user}));_this8.isSaving=!1}else{console.log(`Invalid response received for updating '${_this8.usersSheetName}' ${response.status}`)}}else{if(0<validTextFieldCount){console.log(`Insert new '${_this8.usersSheetName}' row`);formValues[0]=null;const response=yield sheets.api.spreadsheets.values.append({spreadsheetId:_this8.sheetId,range:_this8.usersSheetName,valueInputOption:"USER_ENTERED",insertDataOption:"INSERT_ROWS",values:[formValues]});if(200==response.status){const appendResults=JSON.parse(response.body);if(appendResults&&appendResults.updates){const updatedRange=appendResults.updates.updatedRange,prefix=`${_this8.usersSheetName}!A`;if(updatedRange&&updatedRange.startsWith(prefix)){const newRowName=updatedRange.substr(prefix.length).split(":")[0];console.log(`Successfully created new User row ${newRowName}`);user.Row=parseInt(newRowName);_this8.dispatchEvent(new CustomEvent("updated-user",{detail:user}));_this8.isSaving=!1;const rowField=_this8.fieldForColumnId(_this8.usersRowColumn);if(rowField){_this8.updateField(rowField,newRowName)}else{console.log(`Could not find Row column after creating User`)}}}}else{console.log(`Invalid response received for creating new User ${response.status}`)}}}}else{if(!validMakerLabsId){console.log("Invalid or missing MakerLabs ID")}else{console.log("Invalid or missing form data")}}})()}}customElements.define("view-user-form",ViewUserForm);var _gapiLoaded=!1;class GoogleSigninButton extends LitElement{static get properties(){return{signedIn:{type:Boolean},oauthClientId:{type:String},oauthScopes:{type:String},googleAuthInitialized:{type:Boolean}}}static loadGapi(){return babelHelpers.asyncToGenerator(function*(){if(!_gapiLoaded){const script=document.createElement("script");script.src="https://apis.google.com/js/api:client.js?onload=__gapiCallback";script.setAttribute("async","");document.head.appendChild(script);_gapiLoaded=!0}const gapiLoadedPromise=new Promise(resolve=>window.__gapiCallback=resolve);yield gapiLoadedPromise})()}constructor(){super();this.signedIn=!1;this.googleAuthInitialized=!1}render(){return this.signedIn?html$1`
      ...
      >Login</paper-button>
      `}get cookieWarningDialogMetadata(){return{title:"3rd-party Cookies Must Be Enabled",helpText:"Please enable 3rd-party cookies in your browser. The only such domain used is https://accounts.google.com in order to connect to Google accounts and services."}}updated(){var _this9=this;return babelHelpers.asyncToGenerator(function*(){if(!_this9.googleAuthInitialized&&_this9.oauthClientId&&_this9.oauthScopes){yield GoogleSigninButton.loadGapi();const googleAuthInitializedPromise=new Promise(resolve=>{gapi.client.init({clientId:_this9.oauthClientId,scope:_this9.oauthScopes}).then(()=>{resolve(!0)},error=>{resolve(!1);_this9.dispatchEvent(new CustomEvent("auth-error",{detail:babelHelpers.objectSpread({},_this9.cookieWarningDialogMetadata,{errorText:error.details})}))})});_this9.googleAuthInitialized=yield googleAuthInitializedPromise;const googleAuth=gapi.auth2.getAuthInstance();if(googleAuth){googleAuth.isSignedIn.listen(_this9.handleAuthStateChanged.bind(_this9));const signedIn=googleAuth.isSignedIn.get();_this9.handleAuthStateChanged(signedIn)}else{console.log("could not getAuthInstance")}}else{console.log("gapi not loaded")}})()}handleLogin(){return babelHelpers.asyncToGenerator(function*(){if(!gapi){yield GoogleSigninButton.loadGapi()}const googleAuth=gapi.auth2.getAuthInstance();if(googleAuth){yield googleAuth.signIn({prompt:"select_account"})}else{console.log("could not getAuthInstance")}})()}handleLogout(){return babelHelpers.asyncToGenerator(function*(){const googleAuth=gapi.auth2.getAuthInstance();yield googleAuth.signOut()})()}handleAuthStateChanged(signedIn){var _this10=this;return babelHelpers.asyncToGenerator(function*(){_this10.signedIn=signedIn;var currentUser=null;if(gapi&&gapi.auth2){const googleAuth=gapi.auth2.getAuthInstance(),googleUser=googleAuth.currentUser.get(),authResponse=googleUser.getAuthResponse(!0);if(authResponse&&"access_token"in authResponse){const basicProfile=googleUser.getBasicProfile();currentUser={accessToken:authResponse.access_token,name:basicProfile.getName(),email:basicProfile.getEmail(),photo:basicProfile.getImageUrl()}}}else{console.log("gapi/auth2 library not loaded")}_this10.dispatchEvent(new CustomEvent("currentUser",{detail:currentUser}))})()}}customElements.define("google-signin-button",GoogleSigninButton);setPassiveTouchGestures(!0);setRootPath(AppGlobals.rootPath);class AppShell extends LitElement{static get properties(){return{sheetId:{type:String},machineId:{type:String},accessToken:{type:String},idToken:{type:String},oauthClientId:{type:String},oauthScopes:{type:String},fieldsUrl:{type:String},nextMakerLabsIdUrl:{type:String},fields:{type:Array},userName:{type:String},usersSearchColumns:{type:String},defaultPhotoUrl:{type:String},profilePhotosFolderId:{type:String},query:{type:Object},emptyImageData:{type:String},dialogMetadata:{type:Object}}}constructor(){super();this.emptyImageData="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNgYAAAAAMAASsJTYQAAAAASUVORK5CYII=";this.fields=[];this.dialogMetadata={};this.accessToken=""}render(){return html$1`
    <style>
      app-toolbar {
    ...
    <app-drawer-layout fullbleed force-narrow>
      ...
      <app-header-layout fullbleed>
        ...
        <!-- User Form -->
        ${this.fields&&0<this.fields.length?"":html$1`
            <div class="overlay">
...
`}updated(changedProperties){var _this11=this;return babelHelpers.asyncToGenerator(function*(){if(changedProperties&&changedProperties.has("fields")&&_this11.fields&&0<_this11.fields.length){const form=_this11.shadowRoot.getElementById("form");if(form){form.fields=_this11.fields;var queryStr;if(form.query){if(form.query.Name){form.userName=form.query.Name;queryStr=`where ${form.usersNameColumn} = '${form.userName}'`}if(form.query["MakerLabs ID"]){const makerLabsId=form.query["MakerLabs ID"];queryStr=`where ${form.usersMakerLabsIdColumn} = '${makerLabsId}'`}if(queryStr&&queryStr.length){const datatable=yield form.queryUsers(queryStr);var values=form.getFirstRowValuesFromDatatable(datatable);if(values&&values.length){form.showUserRow(values)}}}const machinesDatatable=yield form.queryActivity("select max("+form.activityMachineIdColumn+") group by "+form.activityMachineIdColumn),machines=[];if(machinesDatatable&&machinesDatatable.table&&machinesDatatable.table.rows&&machinesDatatable.table.cols){for(var rowIdx=0;rowIdx<machinesDatatable.table.rows.length;rowIdx++){const colIdx=0,v=machinesDatatable.table.rows[rowIdx].c[colIdx]&&machinesDatatable.table.rows[rowIdx].c[colIdx].v;if(v){machines.push({type:"machine",style:"",secondaryStyle:"display: none;",imageStyle:"display: none;",iconStyle:"",Name:v,Photo:_this11.emptyImageData})}}}const usersDatatable=yield form.queryUsers(`select ${_this11.usersSearchColumns}`),users=machines.slice(),sections=_this11.fields.map(section=>section.title);if(usersDatatable&&usersDatatable.table&&usersDatatable.table.rows&&usersDatatable.table.cols){for(var rowIdx=0,currentSection;rowIdx<usersDatatable.table.rows.length;rowIdx++){currentSection=0;const user={type:"user",style:"",secondaryStyle:"",imageStyle:"",iconStyle:"display: none;"};for(var colIdx=0,k;colIdx<usersDatatable.table.cols.length;colIdx++){k=usersDatatable.table.cols[colIdx].label;if(0===k.indexOf(sections[currentSection])){k=k.substr(sections[currentSection].length+1);currentSection++}k=k.replace(/\W/g,"_");const v=usersDatatable.table.rows[rowIdx].c[colIdx]&&usersDatatable.table.rows[rowIdx].c[colIdx].v;user[k]=v}if(!("Photo"in user)||!user.Photo){user.Photo=_this11.defaultPhotoUrl}if(user.Name){users.push(user)}}}const search=_this11.shadowRoot.getElementById("search");if(search){search.items=users}}}})()}firstUpdated(){var _this12=this;const database=firebase.database(),auth=this.shadowRoot.getElementById("auth");auth.addEventListener("currentUser",function(){var _ref=babelHelpers.asyncToGenerator(function*(e){if(e.detail){_this12.handleAuthSignIn()}else{_this12.handleAuthSignOut()}});return function(){return _ref.apply(this,arguments)}}());const form=this.shadowRoot.getElementById("form"),search=this.shadowRoot.getElementById("search");form.addEventListener("reset-form",function(){var _ref2=babelHelpers.asyncToGenerator(function*(){search.resetSelectedItem()});return function(){return _ref2.apply(this,arguments)}}());form.addEventListener("updated-user",function(){var _ref3=babelHelpers.asyncToGenerator(function*(e){if(e.detail){const user=e.detail;if(search){search.mergeItem({type:"user",style:"",secondaryStyle:"",imageStyle:"",iconStyle:"display: none;",Row:user.Row,Name:user.Name,Email:user.Email,MakerLabs_ID:user["MakerLabs ID"],Photo:user.Photo})}}});return function(){return _ref3.apply(this,arguments)}}());search.addEventListener("search",function(){var _ref4=babelHelpers.asyncToGenerator(function*(e){if(e.detail){const item=e.detail;switch(item.type){case"user":{if(_this12.onActivityCallback){const latestUserRef=database.ref(`readers/${_this12.machineId}/latestUser`);latestUserRef.off("value",_this12.onActivityCallback);_this12.onActivityCallback=null}form.resetValues();const row=parseInt(item.Row,10),datatable=yield form.queryUsers(`where A = ${row}`),values=form.getFirstRowValuesFromDatatable(datatable);if(values&&values.length){form.showUserRow(values)}break}case"machine":{_this12.machineId=item.Name;const latestUserRef=database.ref(`readers/${_this12.machineId}/latestUser`);if(_this12.pollActivityIntervalId){latestUserRef.off("value",_this12.onActivityCallback);_this12.onActivityCallback=null}form.resetValues();_this12.onActivityCallback=latestUserRef.on("value",function(data){const form=this.shadowRoot.getElementById("form"),user=data.toJSON();if(form&&user){const makerLabsId=user.makerlabsId;form.resetValues();form.showUserObj(user);form.updateFormFromMakerLabsId(makerLabsId)}}.bind(_this12))}}}});return function(){return _ref4.apply(this,arguments)}}())}displayDialog(dialogMetadata){this.dialogMetadata=dialogMetadata;const dialog=this.shadowRoot.getElementById("dialog");if(dialog){dialog.open()}}populateAuthTokens(){const authInstance="object"===typeof gapi&&gapi.auth2&&gapi.auth2.getAuthInstance();if(authInstance){const currentUser=authInstance&&authInstance.currentUser&&authInstance.currentUser.get();if(currentUser){const authResponse=currentUser.getAuthResponse(!0);if(authResponse){if("access_token"in authResponse){this.accessToken=authResponse.access_token}else{console.log("Could not find access_token in Google Sign-In response")}if("id_token"in authResponse){this.idToken=authResponse.id_token;this.loginFirebase()}else{console.log("Could not find id_token in Google Sign-In response")}}}}}fetchUserFields(){var _this13=this;return babelHelpers.asyncToGenerator(function*(){if(_this13.fieldsUrl&&0<_this13.fieldsUrl.length&&_this13.accessToken&&0<_this13.accessToken.length){const response=yield fetch(_this13.fieldsUrl,{mode:"cors",credentials:"omit",headers:{Authorization:`Bearer ${_this13.accessToken}`}});if(200==response.status){_this13.fields=yield response.json()}else{console.log(`Fields JSON request fetch failed with response code: ${response.status}`)}}})()}loginFirebase(){const googleAuthProvider=new firebase.auth.GoogleAuthProvider;var credential=googleAuthProvider.credential(this.idToken);firebase.auth().signInAndRetrieveDataWithCredential(credential).catch(function(error){var errorCode=error.code,errorMessage=error.message,email=error.email,credential=error.credential;if("auth/account-exists-with-different-credential"===errorCode){console.log("You have already signed up with a different auth provider for that email.")}else{console.error(error)}})}handleFirebaseAuthStateChange(user){this.currentUser=user}handleAuthSignIn(){var _this14=this;return babelHelpers.asyncToGenerator(function*(){_this14.populateAuthTokens();setInterval(()=>{_this14.populateAuthTokens();console.log(`Updated accessToken`)},20*(1e3*60));_this14.fetchUserFields();firebase.auth().onAuthStateChanged(_this14.handleFirebaseAuthStateChange.bind(_this14))})()}handleAuthSignOut(){this.currentUser=null;firebase.auth().signOut()}}customElements.define("app-shell",AppShell)});