...
`}get fields(){return this._fields}set fields(fields){this._fieldsChanged(fields,this._fields);this._fields=fields}_fieldsChanged(newValue){var _this6=this;return babelHelpers.asyncToGenerator(function*(){if(newValue){const form=_this6.shadowRoot.getElementById("form");if(form){form.fields=newValue;var queryStr;if(form.query){if(form.query.Name){form.userName=form.query.Name;queryStr=`where ${form.usersNameColumn} = '${form.userName}'`}if(form.query["MakerLabs ID"]){const makerLabsId=form.query["MakerLabs ID"];queryStr=`where ${form.usersMakerLabsIdColumn} = '${makerLabsId}'`}if(queryStr&&queryStr.length){console.log("queryStr");console.log(queryStr);const datatable=yield form.queryUsers(queryStr);var values=form.getFirstRowValuesFromDatatable(datatable);if(values&&values.length){form.showUser(values)}}}const machinesDatatable=yield form.queryActivity("select max("+form.activityMachineIdColumn+") group by "+form.activityMachineIdColumn),machines=[];if(machinesDatatable&&machinesDatatable.table&&machinesDatatable.table.rows&&machinesDatatable.table.cols){for(var rowIdx=0;rowIdx<machinesDatatable.table.rows.length;rowIdx++){const colIdx=0,v=machinesDatatable.table.rows[rowIdx].c[colIdx]&&machinesDatatable.table.rows[rowIdx].c[colIdx].v;if(v){machines.push({type:"machine",style:"",secondaryStyle:"display: none;",imageStyle:"display: none;",iconStyle:"",Name:v,Email:"A Machine!"})}}}const usersDatatable=yield form.queryUsers(`select ${_this6.usersSearchColumns}`),users=machines.slice(),sections=_this6.fields.map(section=>section.title);if(usersDatatable&&usersDatatable.table&&usersDatatable.table.rows&&usersDatatable.table.cols){for(var rowIdx=0,currentSection;rowIdx<usersDatatable.table.rows.length;rowIdx++){currentSection=0;const user={type:"user",style:"",secondaryStyle:"",imageStyle:"",iconStyle:"display: none;"};for(var colIdx=0,k;colIdx<usersDatatable.table.cols.length;colIdx++){k=usersDatatable.table.cols[colIdx].label;if(0===k.indexOf(sections[currentSection])){k=k.substr(sections[currentSection].length+1);currentSection++}k=k.replace(/\W/g,"_");const v=usersDatatable.table.rows[rowIdx].c[colIdx]&&usersDatatable.table.rows[rowIdx].c[colIdx].v;user[k]=v}if(!("Photo"in user)||!user.Photo){user.Photo=_this6.defaultPhotoUrl}if(user.Name){users.push(user)}}}const userSearchBar=_this6.shadowRoot.getElementById("search");if(userSearchBar){userSearchBar.items=users}}}})()}_firstRendered(){var _this7=this;const aware=this.shadowRoot.querySelector("google-signin-aware");aware.handleAuthSignIn=this.handleAuthSignIn;aware.handleAuthSignOut=this.handleAuthSignOut;aware.handleAuthStateChange=this.handleAuthStateChange;const search=this.shadowRoot.getElementById("search"),form=this.shadowRoot.getElementById("form");search.addEventListener("search",function(){var _ref=babelHelpers.asyncToGenerator(function*(e){if(e.detail){const item=e.detail;switch(item.type){case"user":{if(_this7.pollActivityIntervalId){clearInterval(_this7.pollActivityIntervalId);_this7.pollActivityIntervalId=null}form.resetValues();const row=parseInt(item.Row,10),datatable=yield form.queryUsers(`where A = ${row}`),values=form.getFirstRowValuesFromDatatable(datatable);if(values&&values.length){form.showUser(values)}break}case"machine":{item.Name;if(_this7.pollActivityIntervalId){clearInterval(_this7.pollActivityIntervalId);_this7.pollActivityIntervalId=null}form.resetValues();_this7.pollActivityIntervalId=window.setInterval(form.updateFormFromMostRecentScan.bind(form),2e3)}}}});return function(){return _ref.apply(this,arguments)}}())}populateAccessToken(){var accessToken=null;const authInstance="object"===typeof gapi&&gapi.auth2&&gapi.auth2.getAuthInstance();if(authInstance){const currentUser=authInstance&&authInstance.currentUser&&authInstance.currentUser.get();if(currentUser){const authResponse=currentUser.getAuthResponse(!0);if(authResponse){if("access_token"in authResponse){accessToken=authResponse.access_token}}}}return accessToken}handleAuthSignIn(){var _this8=this;return babelHelpers.asyncToGenerator(function*(){_this8.accessToken=_this8.populateAccessToken();setInterval(()=>{console.log(`Update accessToken. Before: ${_this8.accessToken}`);_this8.accessToken=_this8.populateAccessToken();console.log(`After: ${_this8.accessToken}`)},20*(1e3*60));if(_this8.fieldsUrl&&0<_this8.fieldsUrl.length&&_this8.accessToken&&0<_this8.accessToken.length){const response=yield fetch(_this8.fieldsUrl,{mode:"cors",credentials:"omit",headers:{Authorization:`Bearer ${_this8.accessToken}`}});if(200==response.status){_this8.fields=yield response.json()}else{console.log(`Fields JSON request fetch failed with response code: ${response.status}`)}}})()}handleAuthSignOut(){}handleAuthStateChange(){}}customElements.define("app-shell",AppShell)});