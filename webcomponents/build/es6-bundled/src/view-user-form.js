define(["../node_modules/@polymer/lit-element/lit-element.js", "../node_modules/@polymer/iron-icons/iron-icons.js", "../node_modules/@polymer/paper-item/paper-item.js", "../node_modules/@polymer/paper-item/paper-icon-item.js", "../node_modules/@polymer/paper-item/paper-item-body.js", "../node_modules/@polymer/paper-card/paper-card.js", "../node_modules/@polymer/paper-spinner/paper-spinner-lite.js", "../node_modules/@polymer/paper-input/paper-input.js", "../node_modules/@polymer/paper-checkbox/paper-checkbox.js", "../node_modules/@polymer/paper-dropdown-menu/paper-dropdown-menu.js", "../node_modules/@polymer/paper-listbox/paper-listbox.js", "../node_modules/@polymer/paper-radio-group/paper-radio-group.js", "../node_modules/@polymer/paper-radio-button/paper-radio-button.js", "../node_modules/@polymer/paper-fab/paper-fab.js", "../node_modules/@polymer/paper-tooltip/paper-tooltip.js", "../node_modules/@vaadin/vaadin-combo-box/theme/material/vaadin-combo-box.js", "../node_modules/@vaadin/vaadin-date-picker/theme/material/vaadin-date-picker.js", "../node_modules/timeago.js/dist/timeago.js", "./image-file-uploader.js", "./beep.js"], function (_litElement, _ironIcons, _paperItem, _paperIconItem, _paperItemBody, _paperCard, _paperSpinnerLite, _paperInput, _paperCheckbox, _paperDropdownMenu, _paperListbox, _paperRadioGroup, _paperRadioButton, _paperFab, _paperTooltip, _vaadinComboBox, _vaadinDatePicker, _timeago, _imageFileUploader, _beep) {
  ...
  // Vaadin Components
  // Local Components
  class ViewUserForm extends _litElement.LitElement {
    ...
    updated(changedProperties) {
      ...
      if (changedProperties.has("fields")) {
        var datepickers = this.shadowRoot.querySelectorAll( //"vaadin-date-picker-light"
        "vaadin-date-picker");
        ...
        for (var d = 0; d < datepickers.length; d++) {
          ...
          datepicker.checkValidity = value => {
            ...
            return !value || value.length === 0 || parts.length == 3 && parts[0].length == 4 && parts[1].length == 2 && parts[2].length == 2;
          };

          datepicker.i18n.formatDate = d => {
            const yearStr = (d.year + "").replace(/\d+/, y => "0000".substr(y.length) + y);
            const month = d.month + 1;
            const monthStr = (month + "").replace(/\d+/, m => "00".substr(m.length) + m);
            const dayStr = (d.day + "").replace(/\d+/, d => "00".substr(d.length) + d);
      ...
      <form onsubmit="return false;" id="viewUserForm">
        ...
        <div class="mdc-layout-grid">
          <div class="mdc-layout-grid__inner">
            ${this.fields.map(section => _litElement.html`
                <paper-card
      ...
      if (this.fieldIsRadioGroupType(field)) {
        ...
        <div>
          ...
          <paper-radio-group
            ...
            selected="0"
          >
            ${field.choices.map(choice => _litElement.html`
                <paper-radio-button
                  name="${choice}"
    ...
    fieldIsCheckboxType(field) {
      return field.type === "CHECKBOX" || field.type === "VALUE_IN_LIST" && field.choices && field.choices.length == 2 && this.isNoLike(field.choices[0]) && this.isYesLike(field.choices[1]);
    }

    fieldIsRadioGroupType(field) {
      return field.type === "VALUE_IN_LIST" && field.choices && field.choices.length <= 3 && !this.fieldIsCheckboxType(field);
    }

    fieldIsDropdownMenuType(field) {
      return field.type === "VALUE_IN_LIST" && field.choices && field.choices.length > 3;
    }

    fieldIsDatePickerType(field) {
      return field.type == "DATE_AFTER" || field.type == "DATE_BEFORE" || field.type == "DATE_BETWEEN" || field.type == "DATE_EQUAL_TO" || field.type == "DATE_IS_VALID_DATE" || field.type == "DATE_NOT_BETWEEN" || field.type == "DATE_ON_OR_AFTER" || field.type == "DATE_ON_OR_BEFORE";
    }

    fieldIsTextInputType(field) {
      return !this.fieldIsTagIdType(field) && !this.fieldIsAlertsType(field) && (!field.type || field.type == "NUMBER_BETWEEN" || field.type == "NUMBER_EQUAL_TO" || field.type == "NUMBER_GREATER_THAN" || field.type == "NUMBER_GREATER_THAN_OR_EQUAL_TO" || field.type == "NUMBER_LESS_THAN" || field.type == "NUMBER_LESS_THAN_OR_EQUAL_TO" || field.type == "NUMBER_NOT_BETWEEN" || field.type == "NUMBER_NOT_EQUAL_TO" || field.type == "TEXT_CONTAINS" || field.type == "TEXT_DOES_NOT_CONTAIN" || field.type == "TEXT_EQUAL_TO" || field.type == "TEXT_IS_VALID_EMAIL");
    }
    ...
    usersColumnFromFieldTitle(prefixOrTitle, title) {
      ...
      if (this.fields && this.fields.length) {
        ...
        for (var s = 0; s < this.fields.length; ++s) {
          var section = this.fields[s]; // Iterate through fields within the section

          for (var f = 0; f < section.fields.length; ++f) {
            // Extract a candidate field
            var field = section.fields[f]; // If a prefix was provided, check if it matches and the title follows

            if (prefix.length) {
              var prefixStart = field.title.indexOf(prefix);

              if (prefixStart === 0) {
                var titleStart = field.title.indexOf(title, prefixStart + prefix.length + 1 // add a ' ' after prefix
                );

                if (titleStart === 0 && titleStart + title.length === field.length - 1) {
                  return field.name;
                }
              }
    ...
    async handleUserNameChanged(newValue, oldValue) {
      if (newValue) {
        ...
        if (this.usersNameColumn && this.userName) {
          const datatable = await this.queryUsers(`where ${this.usersNameColumn} = '${this.userName}'`);
    ...
    async firstUpdated(changedProperties) {
      ...
      this.updateRecentTagIds(); // Periodically update the Tag ID dropdown with recently scanned tags

      var pollRecentTagIdsIntervalMillis = 10000;
      window.setInterval(this.updateRecentTagIds.bind(this), pollRecentTagIdsIntervalMillis); // repeat forever
    }

    async updateFormFromMostRecentScan() {
      if (this.fields && this.usersMakerLabsIdColumn) {
        //TODO: not hard-coded
        const datatable = await this.queryActivity("select " + this.activityMakerLabsIdColumn + " where " + this.activityTypeColumn + " = 'Signed_In' and " + this.activityMachineIdColumn + " = '" + this.machineId + "' order by " + this.activityTimestampColumn + " desc limit 1");
    ...
    async updateFormFromMakerLabsId(makerLabsId) {
      const datatable = await this.queryUsers("where " + this.usersMakerLabsIdColumn + " = '" + makerLabsId + "'");
    ...
    async updateRecentTagIds() {
      ...
      if (el) {
        ...
        const datatable = await this.queryActivity("select " + this.activityTagIdColumn + ",max(" + this.activityMakerLabsIdColumn + "),max(" + this.activityTimestampColumn + ") where " + this.activityTimestampColumn + " > " + sinceTimestamp + " group by " + this.activityTagIdColumn + " limit 5");
        var items = [];
    ...
    getValuesFromDatatable(datatable, limit) {
      ...
      if (datatable && datatable.table && datatable.table.rows && datatable.table.rows.length > 0 && datatable.table.cols && datatable.table.cols.length > 0) {
        limit = limit < datatable.table.rows.length ? limit : datatable.table.rows.length;
    ...
    querySheet(sheetName, query, numHeaders = 1) {
      ...
      const responseHandlerName = `jsonp_${Date.now()}_${Math.ceil(Math.random() * 100000)}`;
      const query_url = "https://docs.google.com/spreadsheets/d/" + this.sheetId + "/gviz/tq" + "?access_token=" + this.accessToken + "&sheet=" + sheetName + "&headers=" + numHeaders + "&tq=" + queryString + "&tqx=responseHandler:" + responseHandlerName; // Execute the request, return a parsed JSON datatable

      return fetchJsonp(query_url, {
    ...
    queryUsers(query) {
      return this.querySheet(this.usersSheetName, query, this.usersSheetNumHeaders);
    }

    queryActivity(query) {
      return this.querySheet(this.activitySheetName, query, this.activitySheetNumHeaders);
    }
    ...
    updateField(field, val) {
      if (field.name) {
        ...
        if (this.fieldIsCheckboxType(field)) {
          ...
          el.value = val;
        } else if (this.fieldIsTextInputType(field) || this.fieldIsAlertsType(field) || this.fieldIsTagIdType(field)) {
          el.value = val;
    ...
    async populateNextMakerLabsId() {
      if (!this.accessToken || this.accessToken === "null" || this.accessToken === "undefined") {
        console.log("Early exit from populateNextMakerLabsId, no access token set");
        return null;
      }
    ...
    async handleSubmit() {
        ...
        for (var f = 0; f < section.fields.length; ++f) {
          ...
          if (el) {
            if (this.fieldIsCheckboxType(field)) {
              ...
              let value = el.value; // Check for value in text-field, not detected by vaadin-date-picker
              ...
              formValue = el.value; // Check whether any non-empty text value was supplied
      ...
      if (sheets && sheets.api && formValues.length && validMakerLabsId) {
        ...
        if (rowName) {
          ...
          if (response.status == 200) {
            console.log(`Successfully updated '${this.usersSheetName}' row ${rowName}`); // Emit event with user field names and confirmed values

            user.Row = parseInt(rowName);
            this.dispatchEvent(new CustomEvent("updated-user", {
              detail: user
            }));
            this.setIsSaving(false);
          } else {
            console.log(`Invalid response received for updating '${this.usersSheetName}' ${response.status}`);
          }
        } else {
            ...
            if (response.status == 200) {
              ...
              if (appendResults && appendResults.updates) {
                ...
                if (updatedRange && updatedRange.startsWith(prefix)) {
                  const newRowName = updatedRange.substr(prefix.length).split(":")[0];
                  console.log(`Successfully created new User row ${newRowName}`); // Emit event with user field names and confirmed values
              ...
              console.log(`Invalid response received for creating new User ${response.status}`);
            }
          }
        }
      } else {
